        ...
        for isubnet = 1:Subnet(count)
            ...
            if isempty(rare_par_isubnet) == 0
                ...
                for param = rare_par_isubnet
                    ...
                    lengthnum_high(countrare) = length(unique(L));
                    num_high(countrare) = length(strfind(S_outpar{param-(isubnet-1)*length(S_outpar)}(n_species+2,unique(L)),[0,1]));
                    
                    lengthnum_base(countrare) = length(S_outpar{param-(isubnet-1)*length(S_outpar)}(n_species+2,setdiff(1:1000000,unique(L))));
                    num_base(countrare) = length(strfind(S_outpar{param-(isubnet-1)*length(S_outpar)}(n_species+2,setdiff(1:1000000,unique(L))),[0,1]));
                    
                    total_num(countrare) = length(strfind(S_outpar{param-(isubnet-1)*length(S_outpar)}(n_species+2,1:end),[0,1]));
