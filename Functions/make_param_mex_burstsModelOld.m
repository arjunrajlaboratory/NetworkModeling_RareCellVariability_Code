...
function uniquetok = make_param_mex_burstsModelOld(inputfile, outputfile, n_species, iruns)
...
while 1 %Rates
    ...
    if strcmp(tline,'Dependence'), break, end
    inlines{i}=tline;
    i=i+1;
end
...
ratelines = cell(1,2*n_species);            %Rates
lines = cell(1,2*n_species);
ratenames = cell(1,2*n_species);
rate= zeros(1,2*n_species);
...
for i = 1:length(inlines)
    clear R K
    ratelin1 = textscan(inlines{i},'%s%s','delimiter',':');
    ratelin = ratelin1{1};
    lin = ratelin1{2};
...
for currrxn = 1:numrxns
    ...
    % First, split the line into the lhs and the rhs:
    sol1 = textscan(lines{currrxn},'%s%s','delimiter','=');
    LHS = sol1{1};
    RHS = sol1{2};
...
%     end
    for j = 1:length(k_ratenames)
        fprintf(foutc,'  %s = rates[%d];\n',k_ratenames{j},i+d+j-1);
    end
    for k = 1:length(n_ratenames)
        fprintf(foutc,'  %s = rates[%d];\n',n_ratenames{k},i+d+j+k-1);
    end
...
%                     if ion ==1
                        ...
                        dep{currrxn} = find(parmat(:,currrxn) == 1);
                        for jdep = 1:length(dep{currrxn})
                            fprintf(foutc,'+%s*pow(%s,%s)/(pow(%s,%s)+pow(%s,%s))',...
                                on_ratenames{currrxn},uniquetok{dep{currrxn}(jdep)},...
                                n_ratenames{dep{currrxn}(jdep)},...
                                k_ratenames{dep{currrxn}(jdep)},n_ratenames{dep{currrxn}(jdep)},...
                                uniquetok{dep{currrxn}(jdep)},n_ratenames{dep{currrxn}(jdep)});
                        end
%                     else
...
%                  fprintf(foutc,'+%s*%s',b_ratenames{n_species},uniquetok{2*jnew+n_species-1});
