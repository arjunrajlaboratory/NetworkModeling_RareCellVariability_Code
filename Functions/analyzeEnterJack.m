function analyzeEnterJack(n_species, S_outpar, parJ, doplot, inet, thres, isubnet)
if isempty(parJ) == 0
for iPar = 1:length(parJ)
    ...
    for iTimeFinishJ = 1:length(TimeFinishJ)
        ...
        for iSpec = 1:length(InvSpec{iTimeFinishJ})
            FThres{iTimeFinishJ}(iSpec) = TimeStartJ(iTimeFinishJ)-1 +...
                find(S_outpar{param-(isubnet-1)*length(S_outpar)}(InvSpec{iTimeFinishJ}(iSpec),...
                TimeStartJ(iTimeFinishJ):TimeFinishJ(iTimeFinishJ)) >...
                thres(param), 1)-2;
        end
        
        AllDiff{iTimeFinishJ} = diff(sort(FThres{iTimeFinishJ}));
...
if exist('sol')
% name_sol = sprintf('/Volumes/MELANOMAII/Example/sol_EnterJack%d_%d_%d',n_species,inet,isubnet);
name_sol = sprintf('./Example/sol_EnterJack%d_%d_%d',n_species,inet,isubnet);
save(name_sol,'sol')
end
