...
function [maxjackpot,desc,rightskewed,unimodal,samp,samp_time,rand_time] = ...
    analyzeQualTrans_revision(n_species,tcell,threshold,S_outpar)
...
for j = 1:n_species
    cells(j,1:length(S_outpar)) = S_outpar(j+n_species,:) > threshold(j);
end
all_cells = sum(cells);

samp = S_outpar(n_species+1:2*n_species,rand_time:tcell:end);

samp_time = zeros(1,length(S_outpar(n_species+1:2*n_species,rand_time:tcell:end)));
